...
create policy "<table>_viewer_read"
  on public.<table_name>
  for select
  to authenticated
  using (
...
-- Org admins/editors can write
create policy "<table>_org_write"
  on public.<table_name>
  for insert
  to authenticated
  with check (
...
-- RLS: inherit from parent tables
...
| service_role key 泄漏到客户端 | **致命** | `service_role` key 绕过所有 RLS，绝对不能暴露给浏览器。只在服务端使用 |
