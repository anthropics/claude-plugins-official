...
// 提取所有匹配
const allMatches = [...input.matchAll(/<pattern>/g)]
allMatches.forEach(m => {
  console.log(m[0], m.index)
})

// 替换
const result = input.replace(/<pattern>/<flags>, '<replacement>')
...
# compile (recommended for repeated use)
...
- [ ] **排除性**：正则是否正确拒绝所有非预期输入？
- [ ] **锚点**：验证场景是否添加了 `^...$`？
- [ ] **贪婪性**：是否需要非贪婪量词？默认贪婪是否符合预期？
- [ ] **回溯风险**：是否存在嵌套量词或重叠交替？
- [ ] **性能**：对大文本或用户输入是否有超时保护？
- [ ] **边界情况**：空字符串、超长字符串、特殊字符是否处理？
- [ ] **语言兼容**：正则在目标语言中是否语法正确？（Go 不支持断言、Python 命名组语法不同等）
- [ ] **可读性**：复杂正则是否添加了注释或使用 verbose 模式？
- [ ] **转义**：特殊字符是否正确转义？字符串中的反斜杠是否双重转义？
