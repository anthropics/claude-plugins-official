...
每个省的角色提示词在本 skill 的 `templates/` 下，运行前读取对应文件以理解角色定位。输出格式参照 `references/output_schemas.md`。

### 2.1 中书省（草案）

读取 `templates/zhongshu.md`。
...
皇上批准定稿后，依次执行六部。每部读取对应的 `templates/*.md` 角色提示词。输出格式参照 `references/output_schemas.md`。

### 3.1 吏部（分工） — `templates/libu.md`

基于定稿输出角色与任务分配清单。用具体角色，避免抽象占位。
保存 `04_libu.json`。

### 3.2 户部（资源） — `templates/hubu.md`

估算预算、人力、时间与资源缺口。给出区间估计。
保存 `05_hubu.json`。

### 3.3 礼部（流程） — `templates/libu_ritual.md`

输出流程步骤与沟通要点。不得与兵部执行步骤冲突。
保存 `06_libu_ritual.json`。

### 3.4 兵部（执行） — `templates/bingbu.md`

输出有序的执行步骤与里程碑。步骤必须可操作且有顺序。
保存 `07_bingbu.json`。

### 3.5 刑部（风险） — `templates/xingbu.md`

列出风险与规避措施。覆盖高概率与高影响风险。
保存 `08_xingbu.json`。

### 3.6 工部（工具） — `templates/gongbu.md`
