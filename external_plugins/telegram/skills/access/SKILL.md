...
1. Check the environment variable `TELEGRAM_STATE_DIR` (run `echo $TELEGRAM_STATE_DIR`).
2. If set and non-empty, use that value as `$STATE_DIR`.
3. Otherwise, fall back to `~/.claude/channels/telegram`.

All paths below use `$STATE_DIR`. This matches the server's own resolution
logic in `server.ts`.
