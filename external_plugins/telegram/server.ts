...
mcp.setNotificationHandler(
  ...
  async ({ params }) => {
    ...
    const access = loadAccess()
    // Show a preview of what's actually being approved (command, file path, or
    // URL) inline instead of only the tool name. Plain text (no parse_mode is
    // set on sendMessage), so no code fences. Full detail is behind "See more".
    let preview = ''
    try {
