...
| --- | --- | --- |
| `IMESSAGE_APPEND_SIGNATURE` | `true` | Fallback default for appending `\nSent by Claude` to outbound messages. Set to `false` to disable. Overridden per-install by `appendSignature` in `access.json`. |
