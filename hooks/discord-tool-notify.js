...
const SKIP = new Set([
  'mcp__plugin_artifice-discord_artifice-discord__reply',
  'mcp__plugin_artifice-discord_artifice-discord__fetch_messages',
  'mcp__plugin_artifice-discord_artifice-discord__react',
  'mcp__plugin_artifice-discord_artifice-discord__edit_message',
  'mcp__plugin_artifice-discord_artifice-discord__download_attachment',
  'ToolSearch',
  'Glob',
  'Grep',
])
...
process.stdin.on('end', () => {
  ...
  if (SKIP.has(tool) || tool.startsWith('mcp__memory__') || tool.startsWith('mcp__whatsapp__')) {
    process.exit(0)
  }
  ...
  const req = https.request({
    ...
    res.on('end', () => {
      try {
        ...
        if (json.id) {
          fs.writeFileSync('/tmp/deet-tool-notify-last', JSON.stringify({ msg_id: json.id, channel }))
        }
      } catch {}
      process.exit(0)
    })
