...
# Defensive: if the file lacks YAML frontmatter or required fields, treat as no
# active loop and exit cleanly. Without this guard, a stale state file (e.g.
# free-form markdown left behind by an earlier loop that never cleaned up, or
# any file at .claude/ralph-loop.local.md that wasn't generated by /ralph-loop)
# makes `set -euo pipefail` plus the `grep | sed` pipeline below kill the script
# with exit 1 and no stderr — Claude Code reports this as
# "Failed with non-blocking status code: No stderr output" and forces a TUI
# redraw on every Stop event (statusline + INSERT bar + TodoWrite leaking into
# scrollback). Once that file lands in a project, every Claude session in that
# project is silently degraded until the file is removed.
if [[ -z "$FRONTMATTER" ]] || ! echo "$FRONTMATTER" | grep -q '^iteration:' || ! echo "$FRONTMATTER" | grep -q '^max_iterations:'; then
  exit 0
fi

ITERATION=$(echo "$FRONTMATTER" | grep '^iteration:' | sed 's/iteration: *//')
MAX_ITERATIONS=$(echo "$FRONTMATTER" | grep '^max_iterations:' | sed 's/max_iterations: *//')
# Extract completion_promise and strip surrounding quotes if present (optional field)
