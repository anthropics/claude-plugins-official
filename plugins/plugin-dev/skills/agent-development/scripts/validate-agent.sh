...
# Only the first two "---" lines delimit the frontmatter; a later "---" in the
# body is a horizontal rule and must not reopen it.
FRONTMATTER=$(awk '/^---$/{i++; next} i==1' "$AGENT_FILE")
SYSTEM_PROMPT=$(awk '/^---$/{i++; next} i>=2' "$AGENT_FILE")

# Read one field's value out of the frontmatter, with YAML multi-line support.
field() {
  local key="$1"
  printf '%s\n' "$FRONTMATTER" | awk -v key="$key" '
    # The wanted key: take the remainder of the line and start collecting.
    $0 ~ "^" key ":" {
      value = substr($0, length(key) + 2)
      sub(/^[ \t]+/, "", value)
      # "|" and ">" introduce a block scalar; the marker itself is not content.
      if (value ~ /^[|>][0-9+-]*$/) value = ""
      found = 1
      next
    }
    # An indented line continues the value; a new key ends it.
    found && /^[ \t]+/ {
      line = $0
      sub(/^[ \t]+/, "", line)
