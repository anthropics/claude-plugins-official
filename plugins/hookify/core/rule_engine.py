    ...
    def _extract_field(
        ...
        if input_data:
            ...
            if field == "reason":
                ...
                # Support both 'prompt' (Claude Code API) and 'user_prompt' (backward compatibility)
                return input_data.get("prompt", "") or input_data.get("user_prompt", "")

            # Handle Stop event specific fields
            if input_data and input_data.get("hook_event_name") == "Stop":
                if field == "content":
                    # For Stop events, content field maps to last_assistant_message
