...
Put results in `<skill-name>-workspace/` as a sibling to the skill directory. Within the workspace, organize results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Within each eval directory, create one directory per configuration (for example, `with_skill/` and `without_skill/`), and put each execution in a run directory (`run-1/`, `run-2/`, etc.). Don't create all of this upfront — just create directories as you go.
...
- **Improving an existing skill**: the old version. Before editing, snapshot the skill (`cp -r <skill-path> <workspace>/skill-snapshot/`), then point the baseline subagent at the snapshot. Save to `old_skill/run-<M>/outputs/`.
