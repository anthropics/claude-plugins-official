...
If the answer is yes — the engagement already exists, in any form, for any duration — **STOP**. Do not proceed to Step 1 intake. Classifying an existing arrangement is not a planning exercise; it's a liability assessment with remediation implications: back pay (OT, meal/rest premiums), unpaid employer-side payroll tax, benefits eligibility that was denied, unemployment and workers' comp back-exposure, and any remediation/penalty regime the active country's Legal Source Registry records under the `worker-classification-remediation` topic (for the US, concretely: state penalties such as CA's PAGA, and IRS §530 relief analysis — see [`countries/us/knowledge/employment-legal/worker-classification-remediation.md`](../../../countries/us/knowledge/employment-legal/worker-classification-remediation.md)) — and, in strict-test jurisdictions with ongoing work, the prospective exposure of letting it run another day. That analysis is privileged, led by counsel, and coupled with a remediation plan.

Output exactly this block and wait for a response:

> **Out of scope — existing arrangement.**
>
> This skill is designed to analyze a worker engagement *before it starts*, so the classification choice informs how to structure the contract and operations. You've described an arrangement that already exists. Analyzing an existing engagement retroactively is a different exercise: reclassification risk assessment coupled with remediation planning — back-pay exposure, payroll-tax back-exposure, penalty exposure, benefits exposure, the active country's remediation regime (e.g., for the US: IRS §530 relief analysis), and prospective restructuring. That work should be privileged, led by an attorney, and likely coupled with outside-counsel review given the dollar and enforcement exposure.
>
...
> **Research-connector pre-flight.** Before emitting the analysis, call the active country's Search Provider `preflightCheck()` (see [`core/engine/providers/search-provider.interface.md`](../../../core/engine/providers/search-provider.interface.md); for US this resolves to CourtListener/Trellis/Westlaw). Collect this into the reviewer note per CLAUDE.md `## Outputs`: if no connector returns results in Step 2 (or none is configured at run time), record it in the **Sources:** line of the reviewer note — e.g., `not connected — cites from training knowledge; the highest-fabrication pinpoints in classification analyses are test codifications, jurisdiction-specific carve-out subsections (for the US, e.g., CA Lab. Code §§ 2775/2776/2783 — see [`countries/us/providers/citation-provider.us.md`](../../../countries/us/providers/citation-provider.us.md)), element counts in exemptions, and purpose-specific test selection — spot-check those first`. Per-citation `[model knowledge — verify]` tags remain inline. Do not emit a standalone banner above the output.
